...
public class App {
    public static void main( String[] args ) {
        ...
        try {
            ...
            for (Course course : courseList){
                String strSql = "SELECT COUNT(cli_id) FROM ClientCourse WHERE course_id = " + course.getId();
                int count = ((Number)em.createNativeQuery(strSql).getSingleResult()).intValue();
                System.out.println(course.getName() + ", clients: " + count);
            }
        } finally {
            em.close();
            emf.close();
